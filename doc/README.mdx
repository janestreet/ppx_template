...
module%template T_attached = struct
  type ('a : k) t = { x : 'a } [@@kind k = bits32]

  let wrap (x @ p) = { x } [@@kind k = bits32] [@@mode p = nonportable]
  let unwrap ({ x } @ p) = x [@@kind k = bits32] [@@mode p = nonportable]
  let wrap (x @ p) = { x } [@@kind k = bits32] [@@mode p = portable]
  let unwrap ({ x } @ p) = x [@@kind k = bits32] [@@mode p = portable]

  type ('a : k) t = { x : 'a } [@@kind k = bits64]

  let wrap (x @ p) = { x } [@@kind k = bits64] [@@mode p = nonportable]
  let unwrap ({ x } @ p) = x [@@kind k = bits64] [@@mode p = nonportable]
  let wrap (x @ p) = { x } [@@kind k = bits64] [@@mode p = portable]
  let unwrap ({ x } @ p) = x [@@kind k = bits64] [@@mode p = portable]
end
...
# Tips, tricks and short hands

## Recursive functions

Templated functions are expanded to a series of `let ... and ...` bindings, so `let rec`
looks like a set of mutually recursive functions. The compiler doesn't optimise this well,
so it's often better to write a recursive function like so:
...
Note that the inner function has the same name as the outer function, this means the
symbol name produced by the compiler is easily identifiable which aids debugging.

Additionally, higher-order functions written like this should capture the function
parameter, rather than passing it as an argument. This allows the compiler to inline the
outer function and specialize it to the specific function passed in. This still depends
on the compiler deciding to inline the function, but if you don't do this it (currently)
doesn't notice that the function is a constant and therefore can't inline the
higher-order function. This looks something like this:
...
let%template map t f =
  (let rec map t =
    ...
    | hd :: tl -> f hd :: map tl
  in
  map t [@nontail]) [@exclave_if_stack a]
[@@alloc a = (heap, stack)]
```
...
Type substitutions (using `:=`) are supported. For example:
...
Module type substitutions are also supported. For example:
...
By default, poly-attributes like `[@@kind k = (value, bits64)]` omit the mangled suffix
when all values along that axis are defaults (e.g., `value` for kind). The `.explicit`
modifier forces the suffix to always be included:

```ocaml
# let%template id (type (a : k)) (x : a) = x [@@kind.explicit k = (value, bits64)]
val id__value : 'a -> 'a = <fun>
val id__bits64 : ('a : bits64). 'a -> 'a = <fun>
```

Without `.explicit`, the `value` version would be named just `id`.

### `.explicit_plus_unmangled`

The `.explicit_plus_unmangled` modifier is similar, but additionally generates the unmangled
versions for instances whose values on that axis are defaults. While we usually consider
both `value` and `value_or_null` to be default kinds, `explicit_plus_unmangled` only considers
`value` to be the default.

```ocaml
# let%template f (type (a : k)) (x : a @ m) = x
    [@@kind.explicit_plus_unmangled k = (value, bits64)]
    [@@mode m = (global, local)]
val f__value : 'a -> 'a = <fun>
val f : 'a -> 'a = <fun>
val f__value__local : 'a @ local -> 'a @ local = <fun>
val f__local : 'a @ local -> 'a @ local = <fun>
val f__bits64 : ('a : bits64). 'a -> 'a = <fun>
val f__bits64__local : ('a : bits64). 'a @ local -> 'a @ local = <fun>
```

### `.default.explicit`

`.default` floating attributes can also take `.default.explicit` and
`.default.explicit_plus_unmangled` forms.

```ocaml
# module type%template s = sig
    [@@@kind.default.explicit k1 = (value, bits64)]

    type ('a : k1) t1

    [@@@kind.default.explicit k2 = (value, bits64)]
...
module type s =
  sig
    type 'a t1__value
    type ('a, 'b) t2__value__value
    type ('a, 'b : bits64) t2__value__bits64
    type ('a : bits64) t1__bits64
    type ('a : bits64, 'b) t2__bits64__value
    type ('a : bits64, 'b : bits64) t2__bits64__bits64
  end
```

Multiple attributes on the same axis of different explicitness are not allowed.
...
module type%template s = sig
  ...
  [@@@kind.default k2 = (value, bits64)]

  type ('a : k1, 'b : k2) t2
end
```
```mdx-error
Line 6, characters 5-43:
Error: [%template]: explicitness mismatch while merging floating defaults
       (explicit vs default)
```

A poly attribute on some binding completely determines the mangling behavior on that axis,
even if there are previous floating attributes. This includes both the values used in
mangling, and the explicitness.

```ocaml
# module type%template s = sig
    [@@@kind.default.explicit k = (value, value_or_null)]
    [@@@mode.default.explicit l = (global, local)]

    (* Note that:
       - [f] receives mangling only from the kinds in its local [[@@kind]]
         binding, and not also from the default kind
       - [f] is mangled according to the non-explicit scheme for kinds, even
         though there is an explicit [[@@@kind.default]] binding above
       - [f] still receives the default mode mangling, and that mangling respects
         the [.explicit]ness of the floating mode attribute
    *)
    val f : ('a : k1) ('b : k2). 'a @ l -> 'b
    [@@kind k1 = k, k2 = (value, bits64)]
  end
module type s =
  sig
    val f__value__bits64__global : 'a ('b : bits64). 'a -> 'b
    val f__value__bits64__local : 'a ('b : bits64). 'a @ local -> 'b
    val f__global : 'a -> 'b
    val f__value_or_null__bits64__global : 'a ('b : bits64). 'a -> 'b
    val f__local : 'a @ local -> 'b
    val f__value_or_null__bits64__local : 'a ('b : bits64). 'a @ local -> 'b
  end
```
...
- `type`s can be destructively substituted by eventual concrete clients later using
  ...
  attributes (even if we would ultimately like to end up with untemplated types in some
  cases).
