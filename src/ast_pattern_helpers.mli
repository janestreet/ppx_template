...
val ident_expr : 'a Type.t -> ('a Expression.t, expression) pat
...
val kind_expr : (Type.kind Expression.t, expression) pat
...
val binding_poly
  ...
  -> expression_pat:('a Expression.t, expression) pat
  -> mangle:('a Value.t -> 'mangle Type.basic Value.t)
  -> (payload, ('a, 'mangle Type.basic) Binding.t list -> 'c, 'c) Ast_pattern.t

(** Parses a [payload] of the form [[@attr a b c]] as [[ "a"; "b"; "c" ]]. **)
val binding_mono
  :  ('a Type.basic Expression.t, expression) pat
  -> (payload, Expression.Basic.packed list -> 'b, 'b) Ast_pattern.t
