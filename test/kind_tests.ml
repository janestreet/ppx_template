...
let%test_module "no [kind] annotation" =
  (module struct
    module Id : sig
      val%template id : 'a -> 'a
    end = struct
...
let%test_module "empty [kind] annotations" =
  (module struct
    module Id : sig
...
let%test_module "single kind variable" =
  (module struct
    module Id : sig
...
let%test_module "multiple kind variables" =
  (module struct
    module Const : sig
...
let%test_module "multiple bindings" =
  (module struct
    module Fn : sig
      [%%template:
      val const : 'a 'b. 'a -> 'b -> 'a
...
let%test_module "miscellaneous mangling behavior" =
  (module struct
    ...
    let%test_module "doesn't complain about misc. let-bindings" =
      (module struct
...
let%test_module "expression extension" =
  (module struct
...
let%test_module "module extensions" =
  (module struct
    module Id : [%template:
...
let%test_module "module bindings" =
  (module struct
...
let%test_module "type extension" =
  (module struct
...
let%test_module "type declarations" =
  (module struct
...
let%test_module "module declarations" =
  (module struct
    module type%template S = sig
      type 'a id = 'a -> 'a
...
let%test_module "check that we can refer to identifiers we've previously bound" =
  (module struct
          ...
          (fun%template (x, y) ->
            [%test_result: float]
              ((apply2 [@kind float64 bits64])
                 (fun x y -> Float_u.to_float x -. Int64_u.to_float y)
                 (Float_u.of_float x)
                 (Int64_u.of_int64 y))
              ~expect:(x -. Int64.to_float y))
    ;;
