...
[@@@expand_inline
  ...
  module%template _ : sig
    type foo : k [@@kind k = (value, immediate)] [@@deriving compare ~localize]
    and bar : k [@@kind k = (value, immediate)] [@@deriving sexp_of]
  end = struct
    type foo : k = bool [@@kind k = (value, immediate)] [@@deriving sexp_of]
    and bar : k = int [@@kind k = (value, immediate)] [@@deriving compare ~localize]
  end]

let x = {| @@deriving attributes are not duplicated |}

module _ : sig
  type foo : value [@@deriving compare ~localize]
  ...
  and bar : value = int [@@deriving compare ~localize]
  ...
  include struct
    ...
    let compare_foo__local =
      (compare_bool__local : local_ foo -> local_ (foo[@merlin.hide]) -> int)

    and compare_foo__immediate__local =
      (compare_bool__local
       : local_ foo__immediate -> local_ (foo__immediate[@merlin.hide]) -> int)

    and compare_bar__local =
      (compare_int__local : local_ bar -> local_ (bar[@merlin.hide]) -> int)

    and compare_bar__immediate__local =
      (compare_int__local
       : local_ bar__immediate -> local_ (bar__immediate[@merlin.hide]) -> int)
    ;;
